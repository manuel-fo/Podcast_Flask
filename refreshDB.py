...
for rss in rss_links:
    ...
    for episode in parsed.get('episodes'):
        ...
        if(episode == None):
            episode = Episode(title=episode_title, link=episode_link, audio_url=episode_audio_url, time_published=episode_time_published, length=episode_length, podcast=podcast)
